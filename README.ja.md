...
> [!WARNING]
> 開発凍結中: 現在、このプロジェクトの開発は凍結されています。

プロシージャルマテリアルをテクスチャにベイクするための、無料で使いやすいUnreal Engineプラグインです。個人開発者や小規模チームでの利用に最適化されており、シンプルで効率的なワークフローを提供します。

## 機能

*   **多様なプロパティのベイク:** 幅広いマテリアルプロパティをベイクできます。また、各プロパティに応じたサフィックスの自動付与にも対応しています:
    | プロパティ | 自動サフィックス |
    | :--- | :--- |
    | Final Color (最終的な色) | (なし) |
    | Base Color (ベースカラー) | `_BC` |
    | Normal (法線) | `_N` |
    | Emissive Color (エミッシブカラー) | `_E` |
*   **柔軟な出力形式:** ベイクしたテクスチャを **Texture Asset**, **PNG**, **JPEG**, **TGA**, **EXR** ファイルとして保存できます。
    | 出力形式 | 説明 | 備考 |
    | :--- | :--- | :--- |
    | **Texture Asset** | プロジェクト内に `.uasset` を作成します。 | 8-bit, 16-bit 対応。 |
    | **PNG** | `.png` 画像ファイルとして書き出します。 | 8-bit, 16-bit 対応。 |
    | **JPEG** | `.jpg` 画像ファイルとして書き出します。 | **8-bit のみ** (非可逆圧縮)。 |
    | **TGA** | `.tga` 画像ファイルとして書き出します。 | **8-bit のみ** (フォーマット上、チャンネルあたり 16-bit のモードがありません)。 |
    | **EXR** | `.exr` 画像ファイルとして書き出します。 | **16-bit のみ** (リニアカラースペース)。 |
...
1.  **Property to Bake (ベイクするプロパティ):** エクスポートしたいマテリアルチャンネル（例: Base Color, Normal）を選択します。
...
8.  **Output Type (出力タイプ):**
    ...
    *   **PNG, JPEG, TGA, EXR:** テクスチャを画像ファイルとして、PC上の指定した場所にエクスポートします。**JPEG** または **TGA** を選択するとビット深度は 8-bit に、**EXR** を選択すると 16-bit に固定されます。
9.  **Output Path (出力パス):**
...
*   **主な対象バージョン: Unreal Engine 5.5 ～ 5.7。** 本プラグインはこれらのバージョンを対象に開発・テストされています。
*   **Unreal Engine 5.8:** UE 5.8 でも動作します。ただし、エンジン側にマテリアルをテクスチャへベイクする機能が標準搭載されているため、UE 5.8 をお使いの場合はまず標準機能の利用を検討することをおすすめします。
...
*   **Opacity のベイク:** `Opacity` プロパティのベイクは現在利用できません。キャプチャ処理が正しい結果を生成しないため、修正されるまで UI から非表示にしています。
